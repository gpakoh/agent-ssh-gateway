...
3. При запросе к `https://gateway.example.com` передавайте сертификат:
   ...
   curl --cert client.crt --key client.key https://gateway.example.com/api/ssh/connect \
     ...
     -d '{"host":"10.0.0.2","username":"root","password":"..."}'
   ```
...
- Базовый URL: `http://10.0.0.1:8085`
- `Authelia` не требуется.
- Рекомендуемый режим для внутренних агентов и CI в LAN.

### 1.1.1 SSH Key Authentication

Вместо password можно передать private key:

```bash
curl -X POST http://10.0.0.1:8085/api/ssh/connect \
  -H "Content-Type: application/json" \
  -d '{
    "host": "10.0.0.2",
    "port": 22,
...
Поддерживаются: Ed25519, RSA (≥2048), ECDSA, DSS.
...
curl --cert client.crt --key client.key \
  ...
  -H "Content-Type: application/json" \
  -d '{"host":"10.0.0.2","username":"root","password":"<SSH_PASS>"}'
```

### 1.2.1 Автоматизированная выдача сертификатов (для агентов)

Если агент развёртывается впервые, сертификат можно получить через API:

```bash
# 1. Агент генерирует ключ
openssl genpkey -algorithm ed25519 -out agent.key

# 2. Отправляет CSR на gateway (mTLS не требуется для /api/agent/token)
curl -X POST https://gateway.example.com/api/agent/token \
  -H "X-API-Key: $MASTER_API_KEY" \
  -d '{"ttl": 86400}'

# 3. Админ подписывает CSR на LXC 100 и возвращает agent.crt
# 4. Агент использует client.crt + client.key для всех последующих запросов
```

Ротация токена:
```bash
curl -X POST https://gateway.example.com/api/agent/token/refresh \
  -H "X-API-Key: $MASTER_API_KEY"
```
...
### 1.3.1 WebSocket Streaming (для длинных команд и PTY)

Для интерактивных сессий и потоковой передачи вывода используйте WebSocket:

```javascript
const ws = new WebSocket("wss://gateway.example.com/api/ssh/execute/stream");
...
Или через `wscat`:
```bash
wscat -c "wss://gateway.example.com/api/ssh/execute/stream" \
  -H "X-API-Key: $API_KEY" \
  -x '{"session_id":"abc123","command":"top -b -n 1"}'
```
...
```bash
curl -X GET http://10.0.0.1:8085/health
```
...
- `POST /api/ssh/heartbeat` — Ssh Heartbeat
  ...
  Ошибки: 400, 401, 404, 422, 500, 502, 504
  Auth: ApiKeyHeader
- `WS /api/ssh/pty/{session_id}/stream` — Interactive PTY Stream
  Описание: Full-duplex WebSocket PTY session (replaces HTTP polling).
  Обязательные: path.session_id | WS init message: {term, rows, cols}
  Сообщения: {"type":"input","data":"..."} | {"type":"resize","rows":24,"cols":80}
  200: text/event-stream (WebSocket)
  Ошибки: 400, 401, 404, 500
  Auth: ApiKeyHeader (in WS handshake)
...
2.5. Для интерактивной работы (vim, htop): `WS /api/ssh/pty/{session_id}/stream`.
