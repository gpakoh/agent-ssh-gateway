...
s = requests.Session()
s.verify = False
s.post("https://auth.example.com/api/firstfactor", json={
    "username": "agent",
...
# Step 2: SSH Connect
r = s.post("https://gateway.example.com/api/ssh/connect", json={
    "host": "10.255.255.101",
    "port": 22,
    "username": "root",
    "password": "CHANGEME"
...
s.post("https://gateway.example.com/api/ssh/heartbeat", json={"session_id": session_id})
...
r = s.post("https://gateway.example.com/api/context/create", json={
...
r = s.post("https://gateway.example.com/api/git/init", json={
    "context_id": context_id,
    "remote_url": "https://github.com/user/repo.git"  # опционально
})

**Создать коммит:**
r = s.post("https://gateway.example.com/api/git/commit", json={
    "context_id": context_id,
...
s.post("https://gateway.example.com/api/git/backup", params={"context_id": context_id, "backup_name": "before_refactor"})
s.post("https://gateway.example.com/api/git/restore", params={"context_id": context_id})

**Работа с файлами через контекст:**
# Чтение
r = s.post("https://gateway.example.com/api/context/file/read", json={
    "session_id": context_id,  # ← используем context_id как session_id
    "path": "app/main.py"
})

# Редактирование с автокоммитом
r = s.patch("https://gateway.example.com/api/context/file/edit", json={
    "context_id": context_id,
...
**Heartbeat:**
r = s.post("https://gateway.example.com/api/ssh/heartbeat", json={"session_id": session_id})
...
**Запуск:**
r = s.post("https://gateway.example.com/api/jobs/run", json={
    "session_id": session_id,
...
**Статус:**
r = s.get(f"https://gateway.example.com/api/jobs/{job_id}/status")
# → {"job_id": "...", "status": "running|completed|failed|cancelled", "duration": 45.2}

**Результат:**
r = s.get(f"https://gateway.example.com/api/jobs/{job_id}/result")
# → {"status": "completed", "exit_code": 0, "output": "...", "duration": 120.5}

**SSE Stream (реальное время):**
r = s.get(f"https://gateway.example.com/api/jobs/{job_id}/stream", stream=True)
...
**Отмена:**
s.post(f"https://gateway.example.com/api/jobs/{job_id}/cancel")
# → {"status": "cancelled"}
...
**Чтение (JSON):**
r = s.post("https://gateway.example.com/api/file/read", json={
    "session_id": session_id,
...
r = s.get("https://gateway.example.com/api/file/raw", params={
    "session_id": session_id,
    "path": "/media/1TB/Python/NOD_gateway/gateway_client/app/main.py"
})
...
r = s.post("https://gateway.example.com/api/batch/read", json={
    "session_id": session_id,
    "paths": [
        "app/main.py",
        "app/config.py",
...
**Редактирование:**
r = s.patch("https://gateway.example.com/api/file/edit", json={
    "session_id": session_id,
...
r = s.post("https://gateway.example.com/api/file/upload", params={
    "session_id": session_id,
...
r = s.get("https://gateway.example.com/api/file/download", params={
    "session_id": session_id,
    "path": "/remote/path/file.txt"
})
...
r = s.post("https://gateway.example.com/api/file/write", json={
    "session_id": session_id,
...
import base64
content = base64.b64encode(open("local_file.txt", "rb").read()).decode("ascii")
r = s.post("https://gateway.example.com/api/file/upload/json", json={
    "session_id": session_id,
    "path": "/remote/path/file.txt",
    "content": content
})
...
r = s.post("https://gateway.example.com/api/file/upload/stream", 
...
r = s.patch("https://gateway.example.com/api/batch/edit", json={
    "session_id": session_id,
    "files": [
        {
            "path": "app/main.py",
...
r = s.post("https://gateway.example.com/api/project/structure", json={
    "session_id": session_id,
    "path": "/media/1TB/Python/NOD_gateway/gateway_client",
...
r = s.get("https://gateway.example.com/api/project/tree", params={
    "session_id": session_id,
    "path": "/project/path",
    "max_depth": 3
})
...
**Git status:**
r = s.get("https://gateway.example.com/api/git/simple-status", params={
    "session_id": session_id,
    "path": "/project/path"
})
...
**Git diff:**
r = s.post("https://gateway.example.com/api/git/diff", json={
    "session_id": session_id,
    "path": "/project/path",
...
**Rename symbol:**
r = s.post("https://gateway.example.com/api/ast/rename", json={
    "session_id": session_id,
...
**Analyze code:**
r = s.post("https://gateway.example.com/api/ast/analyze", json={
    "session_id": session_id,
    "path": "/project/app/services.py"
})
...
**Конфигурация:**
r = s.get("https://gateway.example.com/api/config/session")
# → {"session_timeout": 3600, "cleanup_interval": 300, "max_sessions_per_ip": 10, "active_sessions": 5}

**Изменить timeout:**
r = s.patch("https://gateway.example.com/api/config/session/timeout", json={"timeout": 7200})
...
**Создать класс:**
r = s.post("https://gateway.example.com/api/scaffold/python-class", json={
    "session_id": session_id,
...
s.post(f"https://gateway.example.com/api/ssh/pty/{session_id}/create")
time.sleep(2)

s.post(f"https://gateway.example.com/api/ssh/pty/{session_id}/input", json={"data": "docker ps\n"})
time.sleep(1)

r = s.get(f"https://gateway.example.com/api/ssh/pty/{session_id}/output")
...
r = s.post("https://gateway.example.com/api/ssh/execute", json={
    "session_id": session_id,
...
r = s.post("https://gateway.example.com/api/validate", json={
    "context_id": context_id,
...
s.post("https://gateway.example.com/api/context/file/open", json={
    "context_id": context_id,
    "path": "app/main.py"
})

**Закрыть файл:**
s.post("https://gateway.example.com/api/context/file/close", json={
    "context_id": context_id,
    "path": "app/main.py"
})

**Обновить позицию курсора:**
s.post("https://gateway.example.com/api/context/cursor", json={
    "context_id": context_id,
    "path": "app/main.py",
    "line": 42,
    "column": 5
...
s.post("https://gateway.example.com/api/context/command", json={
    "context_id": context_id,
    "command": "docker ps",
...
s.post("https://gateway.example.com/api/context/search", json={
    "context_id": context_id,
    "query": "class Context",
    "path": "app"
})
...
**Добавить закладку:**
s.post("https://gateway.example.com/api/context/bookmark", json={
    "context_id": context_id,
    "path": "app/main.py",
    "line": 42,
    "note": "Важная функция"
...
**Удалить закладку:**
s.delete("https://gateway.example.com/api/context/bookmark", params={
    "context_id": context_id,
    "path": "app/main.py",
    "line": 42
})
...
r = s.get(f"https://gateway.example.com/api/context/{context_id}/state")
state = r.json()
...
if ctx["git"]["status"] == "not_initialized":
    ...
    s.post("https://gateway.example.com/api/git/init", json={
        "context_id": context_id,
...
# 6. Завершение
s.delete(f"https://gateway.example.com/api/context/{context_id}")
s.post("https://gateway.example.com/api/ssh/disconnect", json={"session_id": session_id})

### 2.7 Batch Operations (множественные операции)

**Выполнить несколько операций за один запрос:**
r = s.post("https://gateway.example.com/api/batch/execute", json={
    "context_id": context_id,
    "operations": [
        {
            "type": "read",
...
| `http://10.255.255.101:20190` | Запасной #2 |
...
def check_mypy(component):
    ...
    result = s.get(f"https://gateway.example.com/api/jobs/{r.json()['job_id']}/result").json()
...
r = s.post("https://gateway.example.com/api/code/search", json={
    "session_id": session_id,
    "path": "/media/1TB/Python/NOD_gateway/gateway_client",
...
r = s.post("https://gateway.example.com/api/code/generate", json={
...
**Умная вставка кода:**
r = s.post("https://gateway.example.com/api/code/insert", json={
    "context_id": context_id,
    "path": "app/main.py",
...
**Список шаблонов:**
r = s.get("https://gateway.example.com/api/templates")
for t in r.json()["templates"]:
    print(f"{t['id']}: {t['name']} ({t['language']})")

**Использование шаблона:**
r = s.post("https://gateway.example.com/api/templates/render", json={
    "context_id": context_id,
...
s.post("https://gateway.example.com/api/recovery/backup", json={
    "context_id": context_id,
    "name": "before_refactor"
})

**Восстановить из бэкапа:**
s.post("https://gateway.example.com/api/recovery/restore", json={
    "context_id": context_id
})

**Список бэкапов:**
r = s.get("https://gateway.example.com/api/recovery/backups", params={"context_id": context_id})
...
**Анализ проекта:**
r = s.post("https://gateway.example.com/api/analytics", json={
    "session_id": session_id,
    "path": "/media/1TB/Python/NOD_gateway/gateway_client"
})
data = r.json()
...
r = s.post("https://gateway.example.com/api/search/global", json={
    "session_id": session_id,
    "path": "/media/1TB/Python/NOD_gateway/gateway_client",
    "query": "class TransportEngine",
    "use_regex": False,
...
r = s.post("https://gateway.example.com/api/replace/global", json={
    "session_id": session_id,
    "path": "/media/1TB/Python/NOD_gateway/gateway_client",
...
r = s.post("https://gateway.example.com/api/tree", json={
    "session_id": session_id,
    "path": "/media/1TB/Python/NOD_gateway/gateway_client",
    "depth": 2,
    "show_hidden": False
...
**Список серверов:**
r = s.get("https://gateway.example.com/api/servers")
for server in r.json()["servers"]:
    print(f"{server['name']}: {server['host']} ({server['status']})")

**Подключиться к серверу:**
r = s.post("https://gateway.example.com/api/servers/lxc103/connect", json={
    "server_id": "lxc103",
    "password": "CHANGEME"
})
session_id = r.json()["session_id"]
...
r = s.post("https://gateway.example.com/api/servers", json={
    "id": "new_server",
    "name": "New Server",
    "host": "10.0.1.200",
    "port": 22,
...
**Создать снапшот:**
r = s.post("https://gateway.example.com/api/snapshots", json={
    "context_id": context_id,
...
r = s.post("https://gateway.example.com/api/snapshots/restore", json={
    "context_id": context_id,
...
**Список снапшотов:**
r = s.get("https://gateway.example.com/api/snapshots", params={"context_id": context_id})
...
**Создать webhook:**
r = s.post("https://gateway.example.com/api/webhooks", json={
...
print(f"Webhook URL: https://gateway.example.com/api/webhooks/{webhook_id}/deploy")

**Ручной деплой:**
r = s.post(f"https://gateway.example.com/api/webhooks/{webhook_id}/deploy", json={
    "session_id": session_id
})
...
- Открой https://gateway.example.com в браузере
...
| 10.255.255.101 | AI (Docker Host) | root | CHANGEME |
...
curl -O https://gateway.example.com/api/sdk/download
...
python3 -c "
import requests
r = requests.get('https://gateway.example.com/api/sdk/download', verify=False)
...
session = client.ssh_connect("10.255.255.101", username="root", password="CHANGEME")
...
git remote add gitea http://git.example.com:3005/gpakoh/ssh-gateway-ai.git
```
...
# С Gitea
git clone http://git.example.com:3005/gpakoh/ssh-gateway-ai.git
```
...
# → {
#   "10.255.255.101": {"state": "closed", "failure_count": 0},
...
> Домен: https://gateway.example.com
