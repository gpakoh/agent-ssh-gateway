...
class TestC3_CidrCheck:
    ...
    async def test_restore_skips_outside_cidr(self):
        ...
        creds = {"host": "192.168.1.99", "port": 22, "username": "test"}
...
class TestH2_HealthFlags:
    def test_health_response_model(self):
        ...
        inst = ta.validate_python({
            "status": "healthy",
            "version": "0.0.0", "uptime": 1.0,
            "redis": True, "postgres": False, "ready": True,
        })
...
class TestH4_BatchOperationSanitize:
    def test_dangerous_command_raises(self):
        ...
        with pytest.raises(ValueError):
            BatchOperation(
                type="execute", command="rm -rf /", path="/tmp",
            )

    def test_safe_command_passes(self):
        op = BatchOperation(
            type="execute", command="ls -la", path="/tmp",
        )
...
class TestM2_MutualTLS:
    def test_mutualtls_in_security_schemes(self):
        ...
        import app.main as main_module
        schemes = (main_module.app.openapi().get("components", {})
                   .get("securitySchemes", {}))
...
class TestMypyFoundBugs:
    ...
    def test_mypy_exc_to_err_bug(self):
        ...
        assert "for sid, err in zip" in src, (
            "Shutdown block should use 'err' not 'exc'"
        )
