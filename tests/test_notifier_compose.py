...
def test_notifier_compose_defaults_disabled_and_dry_run():
    ...
    assert "GATEWAY_NOTIFIER_PROXY=${GATEWAY_NOTIFIER_PROXY:-}" in env
...
def test_notifier_healthcheck_uses_python_not_external_process_tools():
    service = _load_compose()["services"]["gateway-notifier"]
    healthcheck = service["healthcheck"]["test"]
    rendered = " ".join(str(part) for part in healthcheck)
    assert healthcheck[:3] == ["CMD", "python", "-c"]
    assert "/proc/1/cmdline" in rendered
    assert "app.notifier" in rendered
    assert "pgrep" not in rendered
    assert "ps " not in rendered
...
def test_runbook_real_send_uses_ignored_env_file():
    ...
    assert "GATEWAY_NOTIFIER_PROXY=<http-proxy-url>" in text
