...
```bash
RESP=$(curl -s -X POST https://gateway.example.com/mcp \
...
CONFIRM_RESP=$(curl -s -X POST https://gateway.example.com/mcp \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $MCP_PUBLIC_TOKEN" \
...
```bash
CONFIRM_RESP2=$(curl -s -X POST https://gateway.example.com/mcp \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $MCP_PUBLIC_TOKEN" \
