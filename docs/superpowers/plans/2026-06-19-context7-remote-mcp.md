...
**Goal:** Expose opencode's local Context7 MCP server (`@upstash/context7-mcp`) to ChatGPT as a remote MCP endpoint behind `gateway.example.com/mcp/context7`.
...
Edit nginx config on VPS (above Authelia auth location, inside `server { server_name gateway.example.com; ... }`):
...
SID=$(curl -s -D - -o /dev/null 'https://gateway.example.com/mcp/context7?mcp_token='$MCP_TOKEN \
  -X POST -H 'Content-Type: application/json' -H 'Accept: application/json, text/event-stream' \
...
curl -s 'https://gateway.example.com/mcp/context7?mcp_token='$MCP_TOKEN \
  -H 'Content-Type: application/json' -H 'Accept: application/json, text/event-stream' \
  -H "mcp-session-id: $SID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/list","params":{}}'
```
...
- URL: `https://gateway.example.com/mcp/context7?mcp_token=<generated-token>`
- Auth: None
