...
curl -s https://gateway.example.com/api/capabilities -H "X-API-Key: $API_KEY" | jq .version

# Check version_source
curl -s https://gateway.example.com/api/capabilities -H "X-API-Key: $API_KEY" | jq .version_source
