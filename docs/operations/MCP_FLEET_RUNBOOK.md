...
Six adapters deployed behind nginx on `gateway.example.com` via Streamable HTTP/SSE.
...
All public endpoints: `https://gateway.example.com/<path>?mcp_token=<token>`
...
All routes are in `/etc/nginx/sites-available/gateway.example.com` on VPS `192.0.2.10`.

```bash
ssh root@192.0.2.10
grep -n 'location ^~ /mcp' /etc/nginx/sites-available/gateway.example.com
