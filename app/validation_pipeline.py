...
class ValidationPipeline:
    ...
    async def validate(
        ...
        # Step 1: mypy
        if run_mypy:
            mypy_result = await self._run_mypy(session_id, project_info["mypy_path"], mypy_strict)
        ...
        if run_tests and overall_success:  # Only run tests if mypy passed
            test_result = await self._run_pytest(session_id, test_path or project_info["test_path"])
        ...
        if overall_success:
            ...
            summary = (
                f"❌ Валидация не пройдена: {error_count} ошибок, {warning_count} предупреждений"
            )
    ...
    async def _run_mypy(self, session_id: str, path: str, strict: bool = True) -> ValidationResult:
    ...
    async def _run_pytest(self, session_id: str, test_path: str) -> ValidationResult:
        """Run pytest."""
        start = asyncio.get_event_loop().time()
        ...
        if "passed" in output and "failed" not in output.split("passed")[-1]:
            ...
            return ValidationResult(
                name="pytest",
                status=ValidationStatus.SUCCESS,
                output=f"pytest: {passed} тестов пройдено"
