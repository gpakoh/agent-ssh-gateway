...
def build_api_help(request: Request) -> dict[str, Any]:
    ...
    examples = [
        {
            ...
            "scope": "ssh:connect",
            "curl": 'curl -s -X POST http://localhost:8085/api/ssh/connect \\\n  -H "X-API-Key: $API_KEY" \\\n  -H "Content-Type: application/json" \\\n  -d \'{"host":"<ip-address>","username":"root","password":"secret","port":22}\'',
            "response": '{"session_id":"ses_abc123","host":"<ip-address>","port":22,"username":"root"}',
        },
        {
        ...
        },
        {
            ...
            "scope": "master_key",
            "curl": 'curl -s -X POST http://localhost:8085/api/servers \\\n  -H "X-API-Key: $MASTER_KEY" \\\n  -H "Content-Type: application/json" \\\n  -d \'{"id":"prod-web-1","name":"Production Web 1","host":"<ip-address>","port":22,"username":"deploy","description":"Primary web server"}\'',
            "response": '{"id":"prod-web-1","name":"Production Web 1","host":"<ip-address>","port":22,"username":"deploy","description":"Primary web server"}',
        },
        {
            ...
            "response": '{"servers":[{"id":"prod-web-1","name":"Production Web 1","host":"<ip-address>","port":22,"username":"deploy","status":"unknown"}],"count":1}',
        },
    ]
    ...
    return {
        ...
        "quick_start": {
            ...
            "walkthrough": [
                ...
                },
                {
                    ...
                    "endpoint": "POST /api/ssh/connect",
                    "scope": "ssh:connect",
                    "curl": 'curl -s -X POST http://localhost:8085/api/ssh/connect -H "X-API-Key: $API_KEY" -H "Content-Type: application/json" -d \'{"host":"<ip-address>","username":"root","password":"your-password","port":22}\'',
        ...
        "server_onboarding": {
            ...
            "create_example": {
                "endpoint": "POST /api/servers",
                "body": '{"id":"prod-web-1","name":"Production Web 1","host":"<ip-address>","port":22,"username":"deploy"}',
        ...
        "observability_workflow": {
            ...
            "sections": [
                ...
                {
                    "name": "audit_log",
                    "title": "Audit logging — query recent events",
                    "overview": "The gateway has two independent audit subsystems. Structured JSONL audit events are written for command policy decisions, workspace readonly blocks, and other security events. A REST endpoint exposes the in-memory ring buffer for real-time inspection. Full history lives on disk as JSONL — the endpoint reads the buffer only, not the file.",
                    "endpoints": [
                        {
                            "method": "GET",
                            "path": "/api/admin/audit/recent",
                            "description": "Query recent audit events from the in-memory ring buffer. Master key required. Supports event_type and decision filters, sort order (newest/oldest), and limit (max 1000, default 100). Returns events sorted newest-first by default.",
                            "scope": "master_key",
                            "query_params": [
                                {"name": "limit", "type": "int", "default": 100, "max": 1000, "description": "Max events to return"},
                                {"name": "event_type", "type": "str", "required": False, "description": "Filter by event type (e.g. command.deny, command.execute, workspace.readonly_block)"},
                                {"name": "decision", "type": "str", "required": False, "description": "Filter by decision: allowed, denied, error"},
                                {"name": "sort", "type": "str", "default": "newest", "description": "Sort order: newest or oldest"},
                            ],
                            "response_fields": [
                                {"name": "events", "type": "array", "description": "Audit events (metadata-only, secrets redacted)"},
                                {"name": "total", "type": "int", "description": "Number of events returned"},
                                {"name": "buffer_size", "type": "int", "description": "Total events currently in the ring buffer"},
                            ],
                        },
                    ],
                    "what_is_logged": [
                        "command.execute / command.deny — command policy decisions",
                        "workspace.readonly_block — write attempts blocked by WORKSPACE_READONLY",
                        "policy.evaluate — policy evaluation events",
                        "file.read / file.write / file.edit / file.patch — file operations",
                        "session.connect / session.disconnect — SSH session lifecycle",
                        "auth.check — authentication checks",
                        "mcp.tool — MCP tool invocations",
                    ],
                    "what_is_NOT_logged": [
                        "Command stdout/stderr — only the command and policy decision",
                        "File content — only metadata (path, hashes when applicable)",
                        "Secrets — redacted by AuditEventLogger._redact_event() before storage",
                        "Absolute host paths in actor fields — only fingerprints and opaque IDs",
                    ],
                    "notes": [
        ...
        "recovery_workflow": {
            ...
            "examples": [
                ...
                },
                {
                    ...
                    "response": '{"hosts":[{"host":"<ip-address>","port":22,"key_type":"ssh-ed25519","fingerprint":"SHA256:abc123..."},{"host":"<ip-address>","port":22,"key_type":"ssh-rsa","fingerprint":"SHA256:def456..."}]}',
            ...
            "deploy_notes": {
                ...
                "gated_endpoints": {
                    ...
                    "also_gated": [
                        ...
                        "POST /api/replace/global (non-dry-run)",
                        "POST /api/snapshots",
                    ...
                    "not_gated_by_workspace_readonly": {
                        "description": "These endpoints mutate state but are NOT blocked by WORKSPACE_READONLY. They have their own auth controls (master_key, webhook auth).",
                        "endpoints": [
                            "POST /api/webhooks/{id}/deploy — triggers deploy; guarded by webhook auth, separate control",
                        ],
                        "note": "Webhook deploy is intentionally excluded — it changes deployed state, not workspace files.",
                    },
                    },
                "modes": {
                    "default_readonly": {
                        ...
                        "volume_mounts": "../projects.yaml:/app/projects.yaml:ro, <workspace-root>:<workspace-root>:ro, ssh_keys:/app/ssh_keys:ro",
                    ...
                    "write_test": {
                        ...
                        "description": "Testing only. Sets WORKSPACE_READONLY=false and uses the workspace-rw compose overlay. Only the repo subdir (<repo-root>) is switched to :rw; the parent <workspace-root> stays :ro. Container read_only, projects.yaml, and ssh_keys remain read-only. Revoke after testing.",
                        ...
                        "volume_mounts": "../projects.yaml:/app/projects.yaml:ro, <workspace-root>:<workspace-root>:ro (parent), <repo-root>:<repo-root>:rw (repo), ssh_keys:/app/ssh_keys:ro",
        ...
        "access_control": {
            "title": "Access control gate \u2014 operator loop",
            "overview": "Manage actor approval decisions: inspect, set, and clear. The access-control store is in-memory with optional Redis backing for crash recovery. Redis is not a synchronous source of truth.",
            "truth_notes": [
                "In-memory store is the source of truth for reads and writes.",
                "Redis is best-effort backing for crash recovery only.",
                "clear() deletes from memory and Redis (best-effort). After clear, the actor+IP tuple returns to pending semantics.",
                "No raw tokens, commands, hosts, paths, or session credentials are exposed in these endpoints.",
            ],
            "endpoints": [
                {
                    "method": "POST",
                    "path": "/api/admin/access-control/decision",
                    "auth": "Master key only",
                    "body_fields": ["actor_fingerprint", "source_ip", "decision (allow|deny)", "reason", "ttl_seconds (optional)", "request_id (optional)"],
                    "response": {"decision_id": "dec_...", "key_hash": "...", "decision": "allow|deny", "expires_at": "...", "effective_now": True},
                    "behavior": "Deny immediately disconnects matching sessions. Decision is stored internally as 'allowed'/'denied' (normalized from allow/deny).",
                },
                {
                    "method": "GET",
                    "path": "/api/admin/access-control/recent",
                    "auth": "Master key only",
                    "query_params": [
                        {"name": "limit", "type": "int", "default": 100, "max": 1000, "desc": "Max decisions to return"},
                        {"name": "decision", "type": "string", "optional": True, "desc": "Filter: allowed|denied|pending"},
                        {"name": "sort", "type": "string", "default": "newest", "desc": "newest or oldest"},
                    ],
                    "response": {"decisions": [{"key_hash", "decision", "actor_fingerprint", "source_ip", "reason", "decided_by", "created_at", "expires_at", "ttl_seconds_remaining"}], "total": "N"},
                    "notes": "Reads from in-memory store. Redis is backing, not source of truth.",
                },
                {
                    "method": "POST",
                    "path": "/api/admin/access-control/clear",
                    "auth": "Master key only",
                    "body_fields": ["actor_fingerprint", "source_ip", "reason", "request_id (optional)"],
                    "response": {"key_hash": "...", "cleared": True, "effective_now": True},
                    "behavior": "Deletes decision from memory and Redis (best-effort). After clear, tuple returns to pending semantics. Does NOT kill active sessions.",
                },
            ],
            "smoke_script": "python3 scripts/access_control_smoke.py (set GATEWAY_API_KEY, GATEWAY_URL env vars)",
        },
