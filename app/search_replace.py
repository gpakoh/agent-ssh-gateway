...
class GlobalSearchReplace:
    ...
    async def replace(
        ...
        matches = await self.search(
            session_id, path, search_query, file_pattern, use_regex, case_sensitive, context_lines=0
        )
    ...
    async def count_occurrences(
        ...
        matches = await self.search(
            session_id, path, query, file_pattern, use_regex, case_sensitive, context_lines=0
        )
        return len(matches)
