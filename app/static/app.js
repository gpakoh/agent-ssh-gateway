...
function connectPTY(sessionId, onOutput, onError, onClose) {
    const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
    const ws = new WebSocket(`${protocol}//${window.location.host}/api/ssh/pty/${sessionId}/stream`);

    ws.onopen = () => {
        ws.send(JSON.stringify({ term: "xterm-256color", rows: 24, cols: 80 }));
    };
    ...
    state.pty = {
        sendInput: (data) => ws.send(JSON.stringify({ type: "input", data })),
        resize: (rows, cols) => ws.send(JSON.stringify({ type: "resize", rows, cols })),
        close: () => ws.close(),
    };
}
...
els.connectForm.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        connectPTY(state.sessionId,
            (data) => appendLine(data, 'stdout'),
            (err) => appendLine(err, 'stderr'),
            () => { state.pty = null; },
        );
...
async function runCommand() {
    ...
    try {
        if (state.pty) {
            // Send via persistent PTY channel (interactive shell)
            state.pty.sendInput(command + "\n");
            setControlsEnabled(true);
