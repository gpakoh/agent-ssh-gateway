...
class SSHSessionManager:
    ...
    def __init__(self, session_timeout: int = 300, cleanup_interval: int = 60, max_sessions: int = 100) -> None:
        ...
        async with self._lock:
            for sid, record in list(self._sessions.items()):
                ...
                if idle > self._session_timeout:
                    stale.append((sid, idle))

        for sid, idle in stale:
            logger.info("Closing stale session %s (idle %.0fs)", sid, idle)
    ...
    async def create_session(
        ...
        async with self._lock:
            if len(self._sessions) >= self._max_sessions:
                raise ConnectionError(f"Maximum session limit reached ({self._max_sessions})")

        client = paramiko.SSHClient()
        client.set_missing_host_key_policy(self._get_host_key_policy())
    ...
    async def restore_session(
        ...
        """Restore an existing session from persistent storage."""
        client = paramiko.SSHClient()
        client.set_missing_host_key_policy(self._get_host_key_policy())
        ...
        logger.info("Session %s restored from persistent storage", session_id)
        return session_id
    ...
    async def create_pty_channel(
        ...
        """Create interactive PTY channel for WebSocket streaming."""
        async with self._lock:
            record = self._sessions.get(session_id)
        if not record:
            raise SessionNotFoundError(f"Session {session_id} not found")
        if not record.is_connected():
            raise ConnectionError(f"Session {session_id} disconnected")
        ...
        channel = await loop.run_in_executor(
            None,
            lambda: transport.open_session(),
        )
        await loop.run_in_executor(
            None,
            lambda: channel.get_pty(term, width=cols, height=rows),
        )
        await loop.run_in_executor(
            None,
            lambda: channel.invoke_shell(),
        )
        record.touch()
        return channel
