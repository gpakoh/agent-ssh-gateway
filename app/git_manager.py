...
class GitManager:
    ...
    async def init_repo(self, session_id: str, path: str, remote_url: str | None = None) -> dict:
        ...
        if remote_url:
            commands.append(f"cd {escaped_path} && git remote add origin {shlex.quote(remote_url)}")
    ...
    async def commit(
        ...
        # Commit
        commit_cmd = f"cd {escaped_path} && git commit -m {shlex.quote(message)}"
    ...
    async def create_backup(self, session_id: str, path: str, backup_name: str) -> dict:
        ...
        result = await self._ssh.execute(
            session_id,
            f"cd {escaped_path} && git stash push -m {shlex.quote(backup_name)}",
            timeout=15,
        )
