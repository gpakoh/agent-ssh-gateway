...
function usage(exitCode = 0) {
  const out = [
    ...
    `  node tunnel.mjs ngrok --local http://127.0.0.1:8788 --hostname gateway.example.com`,
    ``,
    `Modes:`,
    ...
    `  --hostname <host>      Public hostname (for stdout URL, e.g. gateway.example.com)`,
    ``,
    `Ngrok-specific:`,
    `  --hostname <host>      Ngrok reserved domain (e.g. gateway.example.com)`,
