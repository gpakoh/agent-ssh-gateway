...
- **MCP public endpoint runbook**: documents VPS nginx/autossh relay setup for `gateway.example.com`. (Session 140)
...
- **NO_PROXY/no_proxy bypass** — `MIMO_EXTRA_NO_PROXY` env var adds local targets (`10.255.255.101`, `10.0.0.x`, etc.) to `NO_PROXY`/`no_proxy` before `mimo run`, preventing proxy-blocked connections. (Session 109.5)
...
- Docker Compose networks changed to `external: true`; gateway container now attaches to `docker_macvlan_example` with static IP `10.255.255.145` for direct LXC→container routing behind nginx.
