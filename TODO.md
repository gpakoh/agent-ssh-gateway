...
2. ⬜ **Correlated ~18-minute CI failures across different Docker runners / phases.** Gitea Actions run `#8141` (PR #70, head `08295c8914d99e2ebdd07d88bfc5f0d02d7cb275`) had green Python 3.11, Python 3.12 and E2E jobs, but `build-and-push` on `runner-docker-e2e` failed while building the `mcp-server` image after ~18m58s (`12:58:56Z` → `13:17:54Z`). At almost the same wall-clock boundary, master run `#8139` (head `44174aea5b312058c19f166c7d8d143bc7940767`) on a different runner, `runner-docker-e2e-2`, had successfully built the same `mcp-server` image in ~3m51s but then its deploy step failed after ~18m03s (`12:59:50Z` → `13:17:53Z`). The one-second-aligned termination strongly suggests a shared timeout/resource/registry/network/storage/supervisor incident or contention rather than the narrow `bash -n` command-policy patch itself. Investigate the common ~18-minute boundary and capture the actual terminal logs/reason from both runners. Closure requires distinguishing runner-specific degradation from a shared infrastructure limit and demonstrating a subsequent controlled build+deploy path that does not hit the same hidden deadline; one incidental green rerun is insufficient evidence by itself.
   - **Partial fix merged (2026-08-20, PR #72):** smoke test hardcoded ports 18085/18087 caused instant bind-address collision when two CI runs shared a runner. Removed `-p` mappings (health probes use `docker exec` anyway). The ~18-minute deploy timeout remains open.
