...
6. ✅ **Координация по `MCP_SCOPE_ENFORCEMENT`/`MCP_CLIENT_SAFE_MODE`** — переключены на живом сервисе (audit→enforce, safe_mode=true) кем-то из другого агента без явной синхронизации. Проверено живым smoke-тестом (initialize → tools/list полный цикл) после переключения и повторно после миграции на общий venv (T81.1) и обновления зависимостей (T80.6) — сервис работает корректно, манифест инструментов ожидаемый (write-тулы скрыты safe_mode, как и задумано в T80/workspace_file_write решении). Не код-фикс — подтверждение, что решение другого агента рабочее и не сломано последующими изменениями.

### T81 — итог

Все 6 пунктов закрыты: T81.1 (мигрировано на общий uv-managed venv + CI-гейт), T81.2 (notifier idle вместо restart-loop), T81.3 (build_sha/build_time реально работают, `scripts/deploy.sh`), T81.4 (owner-фильтрация в Redis dead-letter queue), T81.5 (segment-exact traversal check), T81.6 (подтверждена работоспособность scope enforcement после апдейтов). Все с regression-тестами, ruff+mypy чисто, задеплоено в живой gateway.
