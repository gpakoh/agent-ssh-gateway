...
- **P19.2 — Сервис-слой для god-объектов (в работе)**: `routers/files.py` (1277 строк), `routers/context.py` (700), `routers/jobs.py` (452), `admin_access.py` (262). Бизнес-логика в роутерах; `app/services/` почти пуст.
  - ✅ `app/services/command_gate.py` создан: единая цепочка access-control → effective profile → policy decision → audit (SEC-07). `routers/jobs.py` переведён на него (jobs_run + bulk_execute, −80 строк дубликата). ssh.py и batch.py — те же блоки, но с особенностями (argv/websocket) — вынести след. шагом.
  - Контракт: 3744 passed, ruff clean.
